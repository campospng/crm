...
function ClientEditEditor({ client, close, save, remove }: { client: Client; close: () => void; save: (client: Client) => void; remove: () => void }) {
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm" onMouseDown={close}>
    <form onSubmit={submit} onMouseDown={event => event.stopPropagation()} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[22px] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-950/30">
      ...
      <div className="mt-6 flex items-center justify-between gap-2"><button type="button" onClick={() => { if (window.confirm(`Excluir ${client.company}? Esta ação não pode ser desfeita.`)) { remove(); close(); } }} className="rounded-xl px-3 py-2.5 text-sm font-medium text-rose-600 transition hover:bg-rose-50">Excluir cliente</button><div className="flex gap-2"><button type="button" onClick={close} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600">Cancelar</button><button className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700">Salvar alterações</button></div></div>
    </form>
  </div>;
}
